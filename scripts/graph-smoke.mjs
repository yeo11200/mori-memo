...
try {
  ...
  await graph.getByRole('button', { name: '폴더 요약', exact: true }).click();
  ...
  await expect(page.getByRole('dialog', { name: '지식 그래프 탐색' })).toBeVisible();
  ...
  await expect(page.getByRole('dialog', { name: '지식 그래프 탐색' })).toHaveCount(0);

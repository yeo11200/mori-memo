...
export class CalendarSecureStore<T> implements CalendarStore<T> {
  ...
  async handleGet(): Promise<T | null> {
    ...
    catch (error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return null; throw new Error('캘린더 연결 정보를 읽지 못했습니다.'); }
    if (!this.storage.isEncryptionAvailable()) throw new Error('안전한 캘린더 저장소를 사용할 수 없습니다.');
    try { return JSON.parse(this.storage.decryptString(data)); }
    catch { throw new Error('캘린더 연결 정보를 해독하지 못했습니다.'); }
  }

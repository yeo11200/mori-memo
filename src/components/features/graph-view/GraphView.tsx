...
export const GraphView = ({ notes, folders = [], selectedId, onSelect, onAddLink }: {
  ...
  return <section ref={root} className={'folder-graph' + (expanded ? ' folder-graph--expanded' : '')} role={expanded ? 'dialog' : 'region'} aria-modal={expanded || undefined} aria-label="지식 그래프 탐색" onKeyDown={event => {
    ...
    <div className="folder-graph__heading"><div><small>{noteView ? 'CONNECTED NOTES' : 'FOLDER ATLAS'}</small><h3>{noteView ? (folder ? folder.name + '의 메모' : '메모로 이어진 생각') : '폴더로 보는 지식'}</h3></div><button ref={expandButton} className="wiki__icon-button" aria-label={expanded ? '그래프 축소' : '그래프 확대'} onClick={() => expanded ? handleCollapse() : setExpanded(true)}>{expanded ? <X size={18} /> : <Maximize2 size={17} />}</button></div>
